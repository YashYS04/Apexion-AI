...
from ai_engine.rag_processor import RegulationRAG

class RaceEngineerAgent:
    def __init__(self):
        self.rag = RegulationRAG()
        
        # API Keys for live IBM Granite integration
    ...
    def formulate_query_context(self, question: str, telemetry: Dict[str, Any] = None) -> str:
        """Helper to combine question, telemetry, and settings into a context block"""
    ...
    def get_response(self, question: str, mode: str = "engineering", telemetry: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Main entry point for AI Race Engineer Copilot.
        Returns a structured response containing thought trace, answer, citations, and assumptions.
        """
        ...
        # 2. Check if we should call the live IBM Granite LLM via LangChain/WatsonX
    ...
    def _call_granite_api(self, question: str, mode: str, telemetry: Dict[str, Any], rag_hits: List[Dict], citations: List[str]) -> Dict[str, Any]:
        """
        Calls IBM Granite LLM using LangChain integration.
        For demonstration, if it fails due to network or incorrect API keys, it will gracefully fallback to the local expert.
        """
        try:
            from langchain_ibm import WatsonxLLM
            
            # Setup prompt template
            # In a live setup, we pass structured instructions asking for JSON schema:
            # { "thoughts": [...], "answer": "...", "assumptions": [...], "action": "..." }
            ...
            # Initialize LangChain IBM Granite model
            # Standard model: "ibm/granite-13b-instruct-v2"
            parameters = {
            ...
            try:
                ...
                # Text fallback if LLM output wasn't perfect JSON
                return {
    ...
    def _generate_local_reasoning(self, question: str, mode: str, telemetry: Dict[str, Any], rag_hits: List[Dict], citations: List[str]) -> Dict[str, Any]:
        """
        A local expert F1 reasoning engine that performs telemetry-aware and rule-aware logic.
        Produces highly realistic pit-wall commentary, step-by-step thinking graphs, and alerts.
        """
        q = question.lower()
        
        # Default telemetry mock if not provided
        t = telemetry or {
            "lap": 42,
            "speed": 290,
            "gear": 7,
        ...
        # 2. Pit stop and strategy questions
            ...
            if "vsc" in q or "safety car" in q or "cheap" in q:
                ...
                # General undercut/overcut explanation
                if "undercut" in q:
                    if mode == "engineering":
                        answer = (
